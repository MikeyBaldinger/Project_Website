...
<html lang="en" dir="ltr">
	...
	<?php
		...
		if (strcasecmp($team_input, "roster") == 0) {
			$roster_query = "SELECT * ".
										 "FROM Roster ";
	...
	<ul>
	<?php echo "$team_name"; ?> - <?php echo "$player_name"; ?> <br>
	</ul>
	...
	2019-2020 Player Stats <br>
	<?php echo "$pname"; ?> <br>
	Passing Yards: <?php echo "$pass_yards"; ?> <br>
	Passing Touchdowns: <?php echo "$pass_tds"; ?> <br>
	Interceptions: <?php echo "$int"; ?> <br>
	QB Rating: <?php echo "$rating"; ?> <br><br>
	...
	<?php
		...
		$pos_input = $_POST["input"];

		$pos_query = "SELECT * ".
								 "FROM Quarterback ";
		$pos_results = $connection->query($pos_query);
		if ($pos_results->num_rows > 0) {
			while ($pos = $pos_results->fetch_assoc()) {
			if (strcasecmp($pos_input, "QB") == 0) {
				$pname = $pos['player_name'];
				$pass_yards = $pos['pass_yards'];
				$pass_tds = $pos['pass_tds'];
				$interceptions = $pos['interceptions'];
				$qb_rating = $pos['qb_rating'];
	?>
	...
	<?php
		...
		if ($pos_results->num_rows > 0) {
			while ($pos = $pos_results->fetch_assoc()) {
				if (strcasecmp($pos_input, "RB") == 0) {
					$pname = $pos['player_name'];
					$rush_yards = $pos['rush_yards'];
					$tds = $pos['touchdowns'];
					$fumbles = $pos['fumbles'];
					$carries = $pos['carries'];
	?>
	...
	<?php
		...
		if ($pos_results->num_rows > 0) {
			while ($pos = $pos_results->fetch_assoc()) {
				if (strcasecmp($pos_input, "WR") == 0) {
					$pname = $pos['player_name'];
					$rec_yards = $pos['rec_yards'];
					$rec_tds = $pos['receptions'];
					$receptions = $pos['receptions'];
	?>
	
	<?php echo "$pname"; ?> <br>
	Receiving Yards: <?php echo "$rec_yards"; ?> <br>
	Receiving Touchdowns: <?php echo "$rec_tds"; ?> <br>
	Receptions: <?php echo "$receptions"; ?> <br>
	<br>
